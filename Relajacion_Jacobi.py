...
def jacobi_relaxation(N=100, tolerance=1e-5, L=10, V1=1, V2=-1):
    ...
    phi[int((N/L)*2):int((N/L)*9),int((N/L)*2)] = V1
    phi[int((N/L)*2):int((N/L)*9),int((N/L)*8)] = V2
    ...
    while delta > tolerance:
        ...
        #que los for loops anidados), de manera que recorro toda la grilla, excepto los bordes ([1] hasta [N-1]). Ahora, si 
        ...
        phi_new[int((N / L) * 2):int((N / L) * 9), int((N / L) * 2)] = V1
        phi_new[int((N / L) * 2):int((N / L) * 9), int((N / L) * 8)] = V2
